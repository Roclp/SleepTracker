...
class SleepTrackingService : Service() {
    ...
    private fun initializeService() {
        try {
            ...
            scheduledAudioRecorder=ScheduledAudioRecorder(applicationContext).apply {
    ...
    private fun startTracking() {
        try {
            ...
            if (scheduledAudioRecorder?.startRecording(updateInterval) == true) {
                Log.d(TAG, "scheduledAudioRecorder recording started")
            } else {
                Log.e(TAG, "Failed to start scheduledAudioRecorder recording")
            }
    ...
    private fun stopTracking() {
        try {
            ...
            scheduledAudioRecorder?.stopRecording()
            Log.d(TAG, "Audio ScheduledAudioRecorder stopped")

            motionDetector = null
            audioRecorder = null
            scheduledAudioRecorder = null
...
//                                break
                ...
                if (count.toInt() != 0) {
                    delay((updateInterval * 60 * 1000).toLong())
                } else {
                    count++
                    delay(1000)
                }
    ...
    private fun createNotification(): Notification {
        ...
        val contentText = buildString {
            append("持续时间：$durationText")
            append("\n睡眠状态: ${currentPhase.name.replace("_", " ")}")
            if (alarmTime > 0) {
                append("\n闹钟: ${timeFormat.format(Date(alarmTime))}")
                if (useSmartAlarm) {
                    append(" (智能模式)")
                }
            }
            append("\n体动强度: ${motionDetector?.getMotionLevel()?.format(2) ?: "0.00"}")
            append("\n音频强度: ${audioRecorder?.getMaxAmplitude()?.format(2) ?: "0.00"}")
        }

        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("睡眠监测开启")
